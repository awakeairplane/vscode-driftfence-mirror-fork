...
export class AICustomizationManagementEditor extends EditorPane {
	...
	private createWelcomePage(parent: HTMLElement): void {
		...
		this.welcomeContentContainer.setAttribute('tabindex', '-1');
		...
		subtitle.textContent = localize('welcomeSubtitle', "Define how AI agents behave in your projects. Browse a category or generate new customizations with AI.");
		...
		gettingStartedTitle.textContent = localize('gettingStartedTitle', "Get Started with AI Customizations");
		const gettingStartedDesc = DOM.append(gettingStartedText, $('span.welcome-getting-started-desc'));
		gettingStartedDesc.textContent = localize('gettingStartedDesc', "Tell Copilot about your project and it will set up agents, skills, and instructions for you.");
	...
	private rebuildWelcomeCards(): void {
		...
		for (const category of this.categoryDescriptions) {
			...
			const card = DOM.append(this.welcomeCardsContainer, $('button.welcome-card'));
			...
			this.editorDisposables.add(DOM.addDisposableListener(browseBtn, 'click', (e) => {
				e.stopPropagation();
				this.selectSection(category.id);
			}));

			// "Generate with AI" button (only for prompt-based sections)
			if (category.promptType) {
				...
				this.editorDisposables.add(DOM.addDisposableListener(generateBtn, 'click', (e) => {
					e.stopPropagation();
					if (this.input) {
						this.group.closeEditor(this.input);
					}
	...
	override focus(): void {
		...
		if (this.selectedSection === undefined) {
			this.welcomeContentContainer?.focus();
			return;
		}
