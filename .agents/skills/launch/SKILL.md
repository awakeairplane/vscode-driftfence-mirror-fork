...
- **`agent-browser` must be installed.** It's listed in devDependencies — run `npm install` in the repo root if needed.
...
```bash
# Wait for Code OSS to start (adjust timing as needed)
sleep 10
# If connect fails, retry:
# for i in 1 2 3 4 5; do agent-browser connect 9224 2>/dev/null && break || sleep 2; done
agent-browser connect 9224
agent-browser snapshot -i
```
...
- Check that the port isn't in use by another process: `lsof -i :9224`
